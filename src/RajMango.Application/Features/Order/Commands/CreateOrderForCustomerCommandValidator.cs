...
{
    public class CreateOrderForCustomerCommandValidator : AbstractValidator<CreateOrderForCustomerCommand>
    {
        public CreateOrderForCustomerCommandValidator()
        {
            RuleFor(x => x.TargetCustomerId)
                .GreaterThan(0)
                .WithMessage("A valid customer must be selected.");
        ...
        private static bool HaveValidCrateTypes(IEnumerable<OrderDetailInputDto> details)
        {
            if (details == null) return false;
            return details.All(d => d.CrateType == CrateType.Crate10Kg || d.CrateType == CrateType.Crate20Kg);
        }

        private static bool HaveMinimumTotalWeight(IEnumerable<OrderDetailInputDto> details)
        {
            if (details == null) return false;
